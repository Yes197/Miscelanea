...
plot(varImp(rf_mod))
# observations:
# Item_MRP is the most important variable in predicting the target variable. New variables created by us are also among the top most important ones. Thats why feature engineering is a crucial role in modeling



# 10. XGBOOST
# eta: learning rate, it shrinks the feature weights to make the boosting process more conservative. Low eta value means the model is more robust to overfitting
# gamma: range from 0 to inf. Larger the gamma makes more conservative the algorithm
# max_depth: we specify the maximum dept of a tree
# subsample: proportion of rows that the model will randomly select to grow trees
# colsample_bytree: the ratio of variables randomly chosen to build each tree in the model
...
# Other things that we can do to improve the model prediction
# Transform the target variable to reduce its skewness
# Make independent vs independent variable visualizations to discover some patterns
# We can create separate model for each group of Item_MRP we found and then combine the predictions
# we can group the categories of the categorical variables which are quite similar in shape and size
# train XGBoost model using grid search as done with RandomForest
# use different algorithms such as LightGBM or neurals network models
# model ensembling can also be of great help in improving model's performance